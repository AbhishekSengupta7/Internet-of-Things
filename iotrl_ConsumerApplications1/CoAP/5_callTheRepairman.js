...
const Coffee_Machine_2_5 = "coap://esiremotelab.esi.ei.tum.de:5683/Virtual-Coffee-Machine_2_5";
WoTHelpers.fetch(Coffee_Machine_2_5).then(async (td) => {
    try {
        servient.start().then(async (WoT) => {
            ...
            await thing.subscribeEvent("maintenance", async (maintenanceData) => {  //subscribe to event
                try {
                    //Payload received
                    console.log("maintenanceData: " + await maintenanceData); //payload received
                    ...
                    fs.writeFile(".\\CoAP Log files\\5_callTheRepairman.result.json", JSON.stringify(arr1), function(err, result) {
