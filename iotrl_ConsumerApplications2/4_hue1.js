...
try {
    servient.start().then(async (WoT) => {
     ...
     let thing1 = await WoT.consume(hue_td) 
     
      
     await thing1.invokeAction("setState",{"bri":50,"on":true}); 
        //await thing1.invokeAction("setState",{"effect":"colorloop"}); 

        //thing1.invokeAction("setState",{"effect":"colorloop","sat":0}); }, 3000);
     await thing1.invokeAction("setState",{"effect":"colorloop","sat":254});
     ...
     function delay(time) {
      ...
      let col= await thing1.readProperty("lightInformation"); 
      console.log(col.state.hue)
      
      delay(0100)
      await thing1.invokeAction("setState",{"effect":"none","sat":0});
      

      delay(5000)
      await thing1.invokeAction("setState",{"bri":col.state.bri,"hue":col.state.hue,"sat":col.state.sat}); 
      
      
      //checking if this color is same as at 4999 ms
      //delay(0200)
      //let col1= await thing1.readProperty("lightInformation");
      //console.log(col1.state.hue)


      delay(5000)
      await thing1.invokeAction("setState",{"on":false}); 
        //setTimeout(function(){ thing1.invokeAction("setState",{"hue":65535,"sat":254}); }, 10000);

      setTimeout(function(){ process.exit(0); }, 0500);
  });
}
catch (err) {
